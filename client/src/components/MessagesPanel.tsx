...
export function MessagesPanel() {
  ...
  // Use manual selection only and clean language tags
  const parseTranscriptText = (text: string) => {
    // ALWAYS use the manually selected language from dropdown
    const language = selectedLanguage;
    
    // Remove language tags like (es), (fr), (de), etc.
    let cleanText = text.replace(/\([a-z]{2}\)/g, '').trim();
  ...
  useRTVIClientEvent(
    ...
    useCallback((data: any) => {
      if (data?.text) {
        setMessages(prev => {
          const { language, cleanText } = parseTranscriptText(data.text);
          ...
          if (nonFinalIndex !== -1) {
            ...
            // All chunks are final - only add if this exact text isn't already present
            const existingTexts = updatedChunks.map(chunk => chunk.text);
          ...
          updated[updated.length - 1] = {
            ...
            language: language || lastMessage.language,
          };
  ...
  useRTVIClientEvent(
    ...
    useCallback((data: any) => {
      if (data?.text) {
        setMessages(prev => {
          // Clean language tags from bot text too
          const cleanBotText = data.text.replace(/\([a-z]{2}\)/g, '').trim();
          const chunkId = Date.now().toString() + Math.random();
          const newChunk: TranscriptChunk = {
            id: chunkId,
            text: cleanBotText,
          ...
          if (prev.length === 0 || prev[prev.length - 1].role !== 'bot') {
            // Use the current manual language selection
